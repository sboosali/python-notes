...
class Binop(list, Op):
    ...
    """
    def __init__(self, ops, min_spaces=1):
...
def munge_syntax(syntax):
    ...
    min_spaces = defaultdict(lambda: 1, syntax['min_spaces'])
        ...
        if isinstance(op, list):
            ...
            if are_binops(ops):
                yield Binop(sorted(ops, key=len, reverse=True),
                            min_spaces=min(min_spaces[op] for op in ops))

        if isinstance(op, str):
            if is_binop(op):
                yield Binop(op,
                            min_spaces=min_spaces[op])
            if is_ternop(op):
                yield Ternop(*op.split(),
                              min_spaces=min_spaces[op])
            if is_quatrop(op):
                yield Quatrop(*op.split(),
                               min_spaces=min_spaces[op])
